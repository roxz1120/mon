...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "*🎉 Welcome to the Monster  Ultimate UDP Flooder! 🎉*\n\n"
...
async def Monster (update: Update, context: CallbackContext):
    ...
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Incorrect usage! Use: /Monster  <add|rem> <user_id> <coins>*", parse_mode='Markdown')
        return
...
async def help(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    message = (
        "*🛠️ Monster  VIP DDOS Bot Help Menu 🛠️*\n\n"
